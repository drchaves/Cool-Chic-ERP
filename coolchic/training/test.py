...
from coolchic.component.frame import NAME_COOLCHIC_ENC, FrameEncoder, FrameEncoderOutput
...
from coolchic.training.loss import (
    DISTORTION_METRIC,
    LossFunctionOutput,
    _compute_mse,
    loss_function,
...
class FrameEncoderLogs(LossFunctionOutput):
    ...
    loss_function_output: (
        LossFunctionOutput  # All outputs from the loss function, will be copied is __post_init__
    )
    ...
    def __post_init__(self):
        ...
        for cc_name in typing.get_args(NAME_COOLCHIC_ENC):
            if cc_name in self.detailed_rate_nn:
                self.detailed_rate_nn_bpp[cc_name] = self.detailed_rate_nn.get(cc_name).multiply(
                    1.0 / self.n_pixels
                )
            else:
                # No rate
                self.detailed_rate_nn_bpp[cc_name] = DescriptorCoolChic(
                    arm=DescriptorNN(weight=0.0, bias=0),
                    ifce=DescriptorNN(weight=0.0, bias=0),
                    upsampling=DescriptorNN(weight=0.0, bias=0),
                    synthesis=DescriptorNN(weight=0.0, bias=0),
                )

        # ----- Get sum of latent rate for each cool-chic encoder
        for cc_name in typing.get_args(NAME_COOLCHIC_ENC):
            setattr(self, f"rate_latent_{cc_name}_bpp", self.rate_latent_bpp.get(cc_name, 0))
        ...
        if "detailed_rate_bit" in self.frame_encoder_output.additional_data:
            ...
            for rate in detailed_rate_bit:
                ...
                upscaled_rate.append(F.interpolate(rate, size=self.img_size, mode="nearest"))
                ...
                for cc_name in typing.get_args(NAME_COOLCHIC_ENC):
                    ...
                    for nn_name in [x.name for x in fields(DescriptorCoolChic)]:
                        for weight_or_bias in [x.name for x in fields(DescriptorNN)]:
                            tmp_col_name = (
                                cc_name + "_" + nn_name + "_" + weight_or_bias + col_name_sufix
                            )
                            ...
                            try:
                                tmp_val = (
                                    val[cc_name]
                                    .__getattribute__(nn_name)
                                    .__getattribute__(weight_or_bias)
                                )
                            except KeyError:
                                tmp_val = 0
    ...
    def _format_value(self, value: Union[str, int, float, Tensor], attribute_name: str = "") -> str:
...
def test(
    ...
    encoder_logs = FrameEncoderLogs(
        ...
        encoding_time_second=frame_encoder.encoder_monitor.total_training_time_sec,
        encoding_iterations_cnt=frame_encoder.encoder_monitor.iterations_counter,
