...
from coolchic.component.core.types import NAME_COOLCHIC_ENC, DescriptorCoolChic
from coolchic.io.format.yuv import DictTensorYUV, convert_444_to_420, yuv_dict_clamp
from coolchic.io.types import FRAME_DATA_TYPE, POSSIBLE_BITDEPTH
from coolchic.utils.codingstructure import FRAME_TYPE
from coolchic.utils.termprint import center_str
...
class EncoderMonitor:
    best_loss: float = field(default=1e6, init=False)  # Overall best loss
    ...
    def record_beaten(self, candidate_loss: float) -> bool:
        ...
        Returns:
            bool: True if the candidate loss is better than the best loss (i.e. candidate < best).
        """
        return candidate_loss < self.best_loss

    def set_best_loss(self, new_best_loss: float) -> None:
        """Set the new best loss attribute.

        Args:
            new_best_loss (float): The new best loss obtained during training
        """
    ...
    def increment_time(self, increment: float) -> None:
        if increment < 0:
            raise ValueError(
                f"Cannot increment time with a negative increment. Found increment={increment}"
            )
        self.total_training_time_sec += increment

    def increment_iterations(self, increment: float) -> None:
        if increment < 0:
            raise ValueError(
                f"Cannot increment iterations with a negative increment. Found increment={increment}"
            )
        self.iterations_counter += increment
...
class FrameEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.encoder_monitor = EncoderMonitor()
    ...
    def forward(
        ...
        cc_enc_out = {
            cc_name: cc_enc(**cc_forward_param) for cc_name, cc_enc in self.coolchic_enc.items()
        }

        # Get the rate of each cool-chic encoder
        rate = {cc_name: cc_enc_out_i.get("rate") for cc_name, cc_enc_out_i in cc_enc_out.items()}
        ...
        if self.frame_type == "I":
            ...
            alpha = self.alpha_reparam(cc_enc_out["residue"].get("raw_out")[:, 3:4, :, :])
            ...
            shifted_ref = apply_global_translation(
            ...
            if self.frame_type == "P":
                ...
                beta = self.beta_reparam(cc_enc_out["residue"].get("raw_out")[:, 4:5, :, :])
                pred = beta * self.warper(shifted_ref[0], flow_1) + (1 - beta) * self.warper(
        ...
        if flag_additional_outputs:
            ...
            if self.frame_type in ["P", "B"]:
                ...
                additional_data["masked_pred"] = masked_pred
    ...
    def alpha_reparam(self, raw_value: Tensor) -> Tensor:
        """Reparameterize the raw feature into an alpha feature, in [0, 1]

        Args:
            raw_value: Raw feature [1, 1, H, W]

        Returns:
            Tensor: Alpha parameters weighting the prediction. Shape is
                [1, 1, H, W] and dynamic is [0., 1.]
        """
        return torch.clamp(raw_value + 0.5, 0.0, 1.0)

    def beta_reparam(self, raw_value: Tensor) -> Tensor:
        """Reparameterize the raw feature into a beta feature, in [0, 1]

        Args:
            raw_value: Raw feature [1, 1, H, W]

        Returns:
            Tensor: Beta parameters for the bidirectional pred. Shape is
                [1, 1, H, W] and dynamic is [0., 1.]
        """
        return torch.clamp(raw_value + 0.5, 0.0, 1.0)
    ...
    def get_param(self) -> OrderedDict[NAME_COOLCHIC_ENC, Tensor]:
        ...
        for cc_name, cc_enc in self.coolchic_enc.items():
            param.update({f"coolchic_enc.{cc_name}.{k}": v for k, v in cc_enc.get_param().items()})
    ...
    def set_global_flow(self, global_flow_1: Tensor, global_flow_2: Tensor) -> None:
        ...
        assert global_flow_1.numel() == 2, (
            f"global_flow_1 must have 2 parameters. Found {global_flow_1.numel()}  parameters."
        )

        assert global_flow_2.numel() == 2, (
            f"global_flow_2 must have 2 parameters. Found {global_flow_2.numel()}  parameters."
        )
    ...
    def get_warp_mac_per_pixel(self) -> float:
        ...
        if self.coolchic_enc_param["motion"].flag_hyperlatent:
            idx_first_lat = min(
                self.coolchic_enc_param["motion"].latent_resolution[0],
                self.coolchic_enc_param["motion"].hyperlatent_resolution[0],
            )
        else:
            idx_first_lat = self.coolchic_enc_param["motion"].latent_resolution[0]
    ...
    def save(self, path_file: str) -> None:
        ...
        """
        data_to_save = {
            "encoder_monitor": self.encoder_monitor,
        ...
        for cc_name, cc_enc in self.coolchic_enc.items():
            ...
            data_to_save[f"{cc_name}_nn_q_step"] = cc_enc.get_network_quantization_step()
            ...
            if cc_enc.full_precision_param is not None:
                data_to_save[f"{cc_name}_full_precision_param"] = cc_enc.full_precision_param

        torch.save(data_to_save, path_file)

    def pretty_string(self) -> str:
        """Get a pretty string representing the complexities of
...
def load_frame_encoder(path_file: str) -> FrameEncoder:
    ...
    for cc_name in list_cc_name:
        ...
        frame_encoder.coolchic_enc[cc_name].nn_q_step = loaded_data[f"{cc_name}_nn_q_step"]
        frame_encoder.coolchic_enc[cc_name].nn_expgol_cnt = loaded_data[f"{cc_name}_nn_expgol_cnt"]

        if f"{cc_name}_full_precision_param" in loaded_data:
            frame_encoder.coolchic_enc[cc_name].full_precision_param = loaded_data[
                f"{cc_name}_full_precision_param"
            ]

    frame_encoder.encoder_monitor = loaded_data["encoder_monitor"]
